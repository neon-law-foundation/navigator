...
"Client"), reachable at `{{client_email}}`, for **Neon Law Newleaf** — an uncontested divorce — on the matter referred
to as `{{project_name}}`.
...
`{{product_description}}`. This is a flat fee for the uncontested dissolution, billed once when the matter closes; the
court's filing fees and any service-of-process costs are passed through at cost on top of the flat fee.
