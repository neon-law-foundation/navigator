/*!
 * htmx v2.0.4 — https://htmx.org/
 * Copyright Big Sky Software LLC, licensed under the 2-clause BSD license.
 *
 * Vendored into this repo on 2026-05-26 from:
 *   https://unpkg.com/htmx.org@2.0.4/dist/htmx.min.js
 *
 * Loaded with `defer` from PageLayout. Admin tables use it for
 * partial swaps (`hx-post` + `hx-target` + `hx-confirm`) so a delete
 * removes the row in-place instead of triggering a full reload.
 *
 * Bump procedure: re-download to this path, keep the banner, run
 * the admin browser_e2e tests. Note: htmx 2.x removed IE11 support
 * — we don't ship IE so this is fine.
 */
