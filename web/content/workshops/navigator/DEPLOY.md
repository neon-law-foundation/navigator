...
Six steps, each tagged with the Bloom verb it exercises (the [Anderson & Krathwohl 2001
revision](https://en.wikipedia.org/wiki/Bloom%27s_taxonomy)). You are the operator; the `navigator` CLI is the
instrument.

- **Create** — stand up a billed Google Cloud project and authenticate so `navigator` can act on your behalf.
- **Predict** — run `navigator gcp setup --dry-run` and read every API call the provisioner _would_ make before sending
  one.
- **Identify** — name the twelve Google Cloud APIs the provisioner enables, and why each is needed.
- **Explain** — describe the VPC, the Cloud SQL Postgres instance, and the three storage buckets, and why re-running
  setup is always safe.
- **Execute** — bring up the GKE Autopilot cluster, the static IP, and Fleet membership with one command.
- **Verify** — ship the `web` image and confirm the running service answers `/readyz` with a 200.

## Bring your own project

`navigator gcp setup` assumes the project already exists — it provisions _into_ a project, it does not create one. So
start by creating a project and attaching a billing account (the [project creation
guide](https://cloud.google.com/resource-manager/docs/creating-managing-projects) walks both):
...
Next, let the `navigator` CLI act as you. The provisioner reads [Application Default
Credentials](https://cloud.google.com/docs/authentication/provide-credentials-adc) — the same ADC every Google client
library uses — so one login covers it:
...
Before you change anything, read the plan. `navigator gcp setup` has a `--dry-run` flag — `gcloud` has no universal
equivalent, so we built one — that records every REST call and every `gcloud` shell-out and prints them without sending
traffic or touching your `gcloud` session:

```bash
cargo run -p cli -- gcp setup --project-id your-project-id --dry-run
```

You will see thirteen planned actions: eight REST calls (enable APIs, create the VPC, create the Cloud SQL instance plus
its database and user, create three buckets) followed by five `gcloud` shell-outs for the cluster. Read them, confirm
the project ID and region are yours, then drop `--dry-run` to execute. Every step is idempotent, so a re-run after a
partial failure never produces duplicates.

## The APIs that light up

The first thing a real run does is enable the Google Cloud APIs the rest of the pipeline depends on — twelve of them, in
a single [Service Usage](https://cloud.google.com/service-usage/docs/enable-disable) `batchEnable` call that completes
as one long-running operation. They are `compute`, `sqladmin`, `servicenetworking`, `storage`, `iam`, `container`,
`gkebackup`, `configconnector`, `anthosconfigmanagement`, `logging`, `secretmanager`, and `certificatemanager`. Enabling
an already-enabled API is a no-op, so this step — like every step — is safe to repeat. Nothing else in the run works
until these are on, which is why it goes first.

## Network, SQL, and three buckets

With the APIs on, the `navigator` CLI provisions the data plane over REST:

- A **custom-mode VPC** — no auto-created subnets, regional routing — to hold everything else.
- A **Cloud SQL for Postgres** instance ([docs](https://cloud.google.com/sql/docs/postgres)) running Postgres 15, with a
  `navigator` database and a `web` user. The user's password is generated for you and printed to stderr **exactly once**
  — copy it then, because it is never stored or echoed again. (In `--dry-run` no password is generated or printed.)
- **Three Cloud Storage buckets** ([docs](https://cloud.google.com/storage/docs/creating-buckets)), all uniform
  bucket-level access:
  - `your-project-id-assets` — public marketing photography, the only bucket that ever gets a public binding.
  - `your-project-id-documents` — **private** client documents; this is where `web` writes content-addressed blobs, and
    it is kept separate from assets so confidential data is never co-mingled with anything public.
  - `your-project-id-logs` — long-lived audit and access logs, on the Nearline class because they are written far more
    than read.

Every create call treats an HTTP **409 Conflict** as success — that is Google's "already exists" response — which is
exactly what makes re-running setup safe rather than destructive.

## The cluster comes up

The cluster is the one part the `navigator` CLI drives through `gcloud` rather than REST: the Container API spec is
roughly two hundred lines of JSON, while the [GKE
Autopilot](https://cloud.google.com/kubernetes-engine/docs/concepts/autopilot-overview) one-liner does the same job with
sound defaults. In order, the provisioner reserves a global static IP for the Gateway (so your DNS A record survives a
cluster re-create), creates the Autopilot cluster on the `rapid` release channel with the Secret Manager add-on, then
registers the cluster as a Fleet member. If you point `--config-sync-repo` at your fork, it also applies a [Config
...
`web` fails **loudly** rather than degrading silently: if a required value is missing it crashes on startup with a
structured `enforce_prod_invariants` error naming exactly what is absent. So a pod stuck in `CrashLoopBackOff` is almost
always a missing secret — read it with `kubectl logs deploy/navigator-web -n navigator`. The boot-invariant set is
`DATABASE_URL`, `RESTATE_BROKER_URL`, `NAVIGATOR_OPA_URL`, `NAVIGATOR_STORAGE_BACKEND=gcs`, `SESSION_SECRET`, the
`SENDGRID_*` keys, and `DOCUSIGN_HMAC_KEY`. The always-current list and every variable's meaning live in the canonical
docs — this workshop deliberately does not copy them so it cannot drift. See [`docs/oss-install.md`](/docs/oss-install)
§4 and `.env.example`.

Two rules keep client data safe, and the deploy will not let you skip them:

- **Secrets never live in the manifest tree.** Create the runtime Secret with `kubectl create secret` out-of-band
  (the full `--from-literal` command is in `oss-install.md` §4), so credentials never enter Git.
- **One interface, your choice of source.** NeonLaw keeps values in [Doppler](https://www.doppler.com/) (`dev` for local
  and CI, `prd` rendered into Secret Manager) — see [`docs/secrets-doppler.md`](/docs/secrets-doppler). Doppler is
  optional: a fork can ignore it and use a gitignored `.env` instead. The env-var interface is identical either way.

## Sign-in: bring an OIDC provider, never store passwords

Navigator authenticates people, but it **never stores a password**. There is no password column in the `persons` table
and no hashing crate in the dependency graph — identity is delegated, in full, to an **OIDC-compatible provider** you
bring. Navigator speaks the standard Authorization Code + PKCE flow against it (`/auth/login` → `/auth/callback`) and
discovers every endpoint from `<issuer>/.well-known/openid-configuration`, so no provider URL is hard-coded. Four
environment variables wire it (worked examples for Keycloak, Google, Auth0, and Okta live in `.env.example`):
...
changes** — because email/password is a feature of the _provider_, not of Navigator:

- **Keycloak** — the same open-source IdP the local KIND loop already runs. It serves email/password, self-registration,
  password reset, and email verification from its own hosted pages; point `OAUTH_ISSUER_URL` at a realm and you are
  done. Runs in your cluster, no per-user fee. This is the recommended no-Google path.
- **Auth0 / Okta** — hosted SaaS equivalents: same four env vars, same redirect flow, their own login + reset pages.
...
Navigator's whole external surface is small and legible on purpose — six services, in two kinds. The **platform
services** are the cloud the stack runs on; the **feature vendors** each light up one client-facing capability and stub
out cleanly when you leave them unconfigured. The full catalog, with env prefixes and the per-environment account rule,
is [`docs/third-party-integrations.md`](/docs/third-party-integrations) — this table is the deployer's-eye view of it:
...
Once your instance answers `/readyz`, you can run the firm's whole matter flow against it from your terminal — no
browser, no tailing pod logs — with the `navigator` CLI. It authenticates exactly like `gcloud auth login`: a
browser-loopback OAuth that reuses your instance's existing OIDC session and lands a short-lived (~8h) token on disk,
stored `0600` at `~/.navigator.json` (a single gcloud-style dotfile; set `NAVIGATOR_CONFIG_DIR` to use the legacy
...
in: this CLI is for _your_ install, not ours.

Put the CLI on your `PATH` first. The crate is `cli`; the binary it builds is `navigator`:
...
your own name without forking source. Describe your organization once in a `navigator.yaml` brand pack and let the
`navigator` CLI compile it down:
...
website — it can be just the client portal and workflow engine:

- **`NAVIGATOR_PORTAL_ONLY=true`** mounts only the application surface (`/portal`, auth, `/api`, `/mcp`, the git
  transport, webhooks, the health probes, and the legal pages) and drops the public marketing + Foundation site; `/`
  redirects to `/portal`. Your own website links to your Navigator portal.
- **`NAVIGATOR_TERMS_URL` / `NAVIGATOR_PRIVACY_URL`** point the footer's Terms and Privacy links at the legal pages your
  own attorney publishes on your own site. `brand verify` rejects a portal-only pack with an empty `terms_url` — so you
  never ship NeonLaw's bundled, Nevada-governed terms under your name.
...
This workshop is the narrative; these docs are the source of truth and stay current as the platform evolves — prefer
them when they disagree with anything above:
