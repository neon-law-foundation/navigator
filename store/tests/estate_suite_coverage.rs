...
struct TemplateFrontmatter {
    ...
    #[serde(default)]
    custom_questions: std::collections::BTreeMap<String, serde_yaml::Value>,
}

/// Every frontmatter key that declares a state's wording: a bank-override
/// under `prompts:` or a one-off under `custom_questions:`. A discriminated
/// state must resolve to one of these.
fn frontmatter_prompts(template: &str) -> BTreeSet<String> {
    ...
    let parsed: TemplateFrontmatter =
        serde_yaml::from_str(fm).expect("estate template frontmatter should parse");
    parsed
        .prompts
        .into_keys()
        .chain(parsed.custom_questions.into_keys())
        .collect()
}
