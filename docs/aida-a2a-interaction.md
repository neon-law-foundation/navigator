...
The Foundation's workshop, *Using the Navigator to Rapidly Solve Legal Outcomes*
([`/foundation/workshops/navigator`](../web/content/workshops/navigator/README.md)), is the canonical end-user entry
into exactly this A2A path. Lawyers add AIDA through Gemini's "Add AIDA" connector — no install, no CLI — and every
"tool call" is a Gemini prompt routed through AIDA's skills over A2A. Two behaviors from this doc are the ones a
workshop attendee feels directly:
