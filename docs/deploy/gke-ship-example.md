...
- **CI publishes; nothing builds locally.** A PR merges to `main`; the daily tag flow (`release-tag.yml`) cuts a
  `YY.M.D` tag; the tag push triggers `deploy.yml`, which runs the KIND integration suite and publishes both
  `ghcr.io/neon-law-foundation/navigator-web:YY.M.D` and
  `ghcr.io/neon-law-foundation/navigator-workflows-service:YY.M.D` (plus `latest`).
- **`ship` rolls, it does not build.** It takes a **required** `--tag YY.M.D` (with an optional `.H` suffix — e.g.
  `26.6.25.14` — for an ad-hoc same-day release) naming the published release to roll onto — it never guesses the latest
  tag — confirms the prod Secret satisfies the new binary's boot invariants, pins **both** the `navigator-web` and
  `workflows-service` deployments to that **one** tag, rolls them out together, and re-registers the worker with
...
- **Verify the roll with `/version`.** `GET https://www.<your-domain>/version` reports the running build; the headline
  `release` field is the `YY.M.D` ghcr tag now live.

## The fast path

```bash
# Roll the cluster onto a named published YY.M.D image (both deployments, together).
# --tag is required; ship never guesses the latest tag.
doppler run --project navigator --config prd -- cargo run --release -p cli -- ship --tag 26.6.23

# Print every command, run nothing.
doppler run --project navigator --config prd -- cargo run --release -p cli -- ship --tag 26.6.23 --dry-run
