...
//! 4. Hit `/auth/login?return_to=/lawyer/people`, follow the redirect
//!    back to `/auth/callback`, then hit `/lawyer/people` with the
...
async fn full_oidc_flow_upserts_person_and_allows_lawyer() {
    ...
    let session_cookie = complete_oauth_flow(&app, &idp, "/lawyer/people").await;
    ...
    // 3. The embedded policy allows Lawyer to view /lawyer/people.
    let resp = app
        .oneshot(
            Request::builder()
                .uri("/lawyer/people")
