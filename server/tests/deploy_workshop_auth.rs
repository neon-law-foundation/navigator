...
fn local_rauthy_presenter_accounts_match_the_dev_seed_contract() {
    ...
    // then email → the disposable Person and Henderson participation. Keep the
    ...
    for token in [
        "DEV_PORTFOLIO_LAWYER_EMAIL: &str = \"lawyer@neonlaw.com\"",
        "\"client@neonlaw.com\"",
        "lawyer_id, \"paralegal\"",
    ] {
        assert!(
            seed.contains(token),
            "the dev seed must keep the Rauthy presenter mapping and Henderson lawyer participation: missing `{token}`",
        );
    }
}
