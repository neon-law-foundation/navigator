...
//!
//! It deliberately exercises [`portal::billing::BillingProvider::ensure_contact`]
...
//! notably the ship verify, which runs under the production environment.
//! Run locally against the demo company through a gitignored `.env`:
//!
//! ```bash
...
use portal::billing::{BillingProvider, ContactRequest, XeroBillingProvider};
use portal::xero_auth::XeroClientCredentials;
...
/// so a developer can source `.env` to drive the live test; CI still
/// overrides with the sandbox names.
...
async fn sandbox_resolves_a_contact_idempotently() {
    ...
    // ambient-credentials `cargo test` (e.g. a shell holding a production
    // deployment's decrypted values). Only the dedicated nightly job and an
