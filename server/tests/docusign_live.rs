...
//! CI/CD — they are on-demand grounding tools, run by hand with an
...
//!    allowance. Run with the production deployment's DocuSign values
//!    decrypted into the process env for this one command (`sops exec-env`
...
//!    to our website and so grounds the HMAC end to end. Demo credentials
//!    from `.env` are free + watermarked; the production deployment's are
//!    the real artifact. The recipient email has NO default, so the test
//!    can never silently email anyone:
//!
//!    ```bash
...
use portal::docusign_auth::DocuSignJwtAuth;
use portal::signature::{
    SignatureField, SignatureFieldKind, SignatureManifest, SignatureProvider, SignatureRecipient,
};
use portal::signature_render::expand_signatures;
...
/// name, so the same test drives demo or production from the explicitly
...
async fn emailed_envelope_to_a_real_signer() {
    ...
    let Some(provider) = portal::signature::DocuSignSignatureProvider::from_env() else {
