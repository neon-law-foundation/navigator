...
async fn admin_sees_the_confirmation_screen() {
    ...
    let resp = f
        ...
        .oneshot(
            Request::builder()
                .uri(format!("/lawyer/documents/{}/expunge", f.doc_id))
...
async fn admin_post_expunges_and_shows_the_audit_row() {
    ...
    assert_eq!(
        location,
        format!("/lawyer/documents/{}/expunge?record={}", f.doc_id, row.id)
    );
...
async fn unknown_category_is_rejected_without_expunging() {
    ...
    let location = resp
        ...
        .to_string();
    assert!(
        location.starts_with(&format!("/lawyer/documents/{}/expunge?error=", f.doc_id)),
        "location: {location}"
    );
    let form_again = f
        .app
