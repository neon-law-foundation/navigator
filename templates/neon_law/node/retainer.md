...
`{{person__client.name}}` (the "Client"), reachable at `{{person__client.email}}`, for **Neon Law Node** — an
on-chain attorney attestation — on the matter referred to as `{{project__engagement.name}}`.
