...
/// `archives`'s `ArchivesService` (which also talks to the notifier directly,
/// for a mrkdwn digest), and unlike `billing-workflows`'s `BillingCanaryService`
/// which routes a plain-text notice through the `EmailService` ops-delivery
/// adapter: the heartbeat has no email framing to render.
#[derive(Clone)]
...
#[restate_sdk::workflow(name = "Heartbeat")]
impl HeartbeatService {
    #[restate_sdk::handler]
    async fn run(
        &self,
