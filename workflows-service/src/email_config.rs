...
//!
//! Mirrors `portal::email::select_backend` in shape but returns a
...
//! requires the `web` crate's store handle).
//!
...
pub enum EmailConfigError {
    ...
    MissingApiKey,
    #[error("{0} requires NAVIGATOR_EMAIL_BACKEND=sendgrid and matching SendGrid credentials")]
    Deployment(String),
}

/// Prevent a dev-profile worker from silently capturing client-facing mail.
pub fn validate_for_deployment<F: Fn(&str) -> Option<String>>(
    environment: store::DeploymentEnvironment,
    get: F,
) -> Result<(), EmailConfigError> {
    if environment == store::DeploymentEnvironment::Dev
        && store::deployment::ci_harness_enabled(&get)
    {
        return Ok(());
    }
    let credential_environment = get(store::deployment::CREDENTIAL_ENVIRONMENT);
    if get("NAVIGATOR_EMAIL_BACKEND").as_deref() != Some("sendgrid")
        || get("SENDGRID_API_KEY").is_none_or(|value| value.is_empty())
        || credential_environment.as_deref() != Some(environment.as_str())
    {
        return Err(EmailConfigError::Deployment(
...
#[cfg(test)]
mod tests {
    use super::{select_backend, validate_for_deployment, EmailConfigError};
    ...
    fn select_backend_requires_api_key_for_sendgrid_backend() {
        assert!(matches!(
            select_backend(lookup(&[("NAVIGATOR_EMAIL_BACKEND", "sendgrid")])),
    ...
    #[test]
    fn normal_dev_requires_real_non_production_sendgrid() {
        let environment = store::DeploymentEnvironment::Dev;
        assert!(validate_for_deployment(environment, lookup(&[])).is_err());
        assert!(validate_for_deployment(
            environment,
            lookup(&[
                ("NAVIGATOR_EMAIL_BACKEND", "sendgrid"),
        ...
        assert!(validate_for_deployment(
            ...
            lookup(&[
                ...
                ("NAVIGATOR_EMAIL_BACKEND", "capturing"),
            ])
        )
        .is_ok());
    }
