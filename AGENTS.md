...
- **Use the KIND loop for full-stack local testing, then clean it up.** `cargo run --release -p cli -- start-dev-server`
  ...
  **under Doppler with `.devx/env` sourced after** (the KIND wiring must win) — `web` crash-loops on `.devx/env` alone
  ...
  [`docs/observability.md`](docs/observability.md). Keep local services up only while actively testing the current PR;
  before handoff, stop the stack and clean task-owned Docker/build artifacts.
...
- **Clean up task resources before ending.** After creating or updating a PR, clean worktree build artifacts created by
  Cargo, stop task-owned KIND/Docker/browser processes, and prune task-created Docker build cache/images. Never prune
  Docker volumes without explicit user approval. → [`docs/agent-workflows.md`](docs/agent-workflows.md#resource-cleanup)
...
agent reply. The artifacts do not have to come from Cursor Cloud: they can be generated by the agent's local browser,
Playwright, a screen recorder, or any other trustworthy preview path. Prefer one short demo video or GIF plus the
clearest screenshots that prove the changed states. Use artifact HTML tags the PR tool understands, for example:
...
If the PR tool rewrites local artifact references to hosted links, that is fine; the important part is that reviewers
can open the visual evidence from the PR. If the artifact cannot be embedded directly, include the generated path and
briefly explain why, but still produce the walkthrough whenever practical.
