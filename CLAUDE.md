...
- **PR flow** — [`.github/workflows/ci.yml`](.github/workflows/ci.yml). Runs only on every `pull_request` targeting
  ...
  does **not** run here. **Auto-merge** is a GitHub-native repo setting, not a workflow. Enable it per PR with the CLI
  `gh pr merge --auto --squash`; GitHub squash-merges the PR the moment this `ci.yml` run goes green, which is why three
