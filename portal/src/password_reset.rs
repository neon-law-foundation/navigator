...
use store::email_tokens::PURPOSE_PASSWORD_RESET;
...
pub(crate) fn mint_csrf_with(
    ...
    cookies.add(csrf_cookie_with_ttl(
        cookie_name,
        signed,
        secure,
        TOKEN_TTL_MINUTES,
...
/// Build a signed double-submit CSRF cookie under the given name and
/// lifetime. One builder so every CSRF cookie (account recovery, workshop
/// certificate, …) shares the same HttpOnly / SameSite=Lax hardening.
pub(crate) fn csrf_cookie_with_ttl(
    name: &'static str,
    value: String,
    secure: bool,
    ttl_minutes: i64,
    ...
    c.set_path("/");
    c.set_max_age(tower_cookies::cookie::time::Duration::minutes(ttl_minutes));
    c
}
...
async fn request_form(State(s): State<AuthState>, cookies: Cookies) -> Response {
    ...
    webapp::auth_pages::password_reset_request(&csrf, None).into_response()
}
...
async fn request_submit(
    ...
    webapp::auth_pages::password_reset_sent().into_response()
}
...
async fn try_send_reset(s: &AuthState, email: &str) -> anyhow::Result<()> {
    ...
    let Some(person) = find_person_by_email(&s.surreal, email).await? else {
        return Ok(());
    };
...
async fn send_reset_email(s: &AuthState, person: &store::persons::Person, plaintext: &str) {
...
async fn send_google_sign_in_email(s: &AuthState, person: &store::persons::Person) {
    let base_url = workflows::email::base_url_from_env();
...
async fn new_form(
    ...
    let now = Utc::now();
    match store::email_tokens::validate(&s.surreal, &q.token, PURPOSE_PASSWORD_RESET, now).await {
        Ok(Some(_)) => {
            let csrf = mint_csrf(&s, &cookies);
            webapp::auth_pages::password_reset_new(&q.token, &csrf, None).into_response()
        }
        Ok(None) => webapp::auth_pages::invalid_link().into_response(),
        Err(e) => {
...
async fn new_submit(
    ...
    let now = Utc::now();
    let token_row =
        match store::email_tokens::validate(&s.surreal, &form.token, PURPOSE_PASSWORD_RESET, now)
            .await
        {
            Ok(Some(row)) => row,
    ...
    if let Some(err) = password_policy_error(&form.password, &form.confirm) {
        ...
        return (
            ...
            webapp::auth_pages::password_reset_new(&form.token, &csrf, Some(err)),
        )
            .into_response();
    }
    ...
    let local_id = match admin.lookup_by_email(&token_row.email).await {
        ...
        Ok(_) => {
            ...
            return webapp::auth_pages::invalid_link().into_response();
        }
        Err(e) => {
...
/// Case-insensitive person lookup by email, so a reset request with
/// different casing still finds the row.
