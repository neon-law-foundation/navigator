...
//!    (a content-addressed document `assets` row; the bytes land in
//!    `cloud::StorageService`, i.e. GCS in prod). This is the primary,
//!    must-succeed write.
//! 2. **Commits** the bytes into the Project's append-only repo authored
//!    as the acting person ([`repos::RepoStore::commit_as`]), so
//!    `git log` is a matter audit surface (design §7).
...
//!    record is reconstructable from the lake (GCS objects + this event)
...
/// commit + lake capture are best-effort.
///
/// # Errors
...
pub async fn record_document(
    ...
    // (1) Durable primary write — bytes to object storage + the `assets` row.
    let ingested = documents::ingest_bytes(surreal, storage, args, bytes).await?;
    ...
    .await
    {
        // The repo commit is a best-effort audit layer; its oid is captured
        // into the data lake, not stamped back on the asset row.
        Ok(_) => {}
        Err(e) => {
            tracing::warn!(error = %e, asset_id = %ingested.asset_id, project_id = %args.project_id,
...
/// key — so this **does not** create an `assets` row or touch that key.
/// It only adds the repo-history + lake audit layer.
///
...
pub async fn commit_files(
    ...
    match do_commit_and_capture(
        surreal, storage, project_id, author, source, kind, message, files, None,
    )
    .await
    {
        Ok(oid) => oid,
...
/// Optional link from a commit event back to the `assets` row it filed.
/// Absent for surfaces that store bytes outside the `assets` table
/// (e-sign).
struct DocLink<'a> {
    asset_id: Uuid,
    blob_sha256: &'a str,
...
async fn do_commit_and_capture(
    ...
    let event = CommitEvent {
        ...
        kind,
        source,
        asset_id: link.as_ref().map(|l| l.asset_id),
        blob_sha256: link.as_ref().map(|l| l.blob_sha256),
        committed_at: &committed_at,
    };
    capture_commit_event(storage, &event).await?;
...
/// the `archives` snapshot and `email_events` schemes. `asset_id` /
...
fn encode_commit_parquet(event: &CommitEvent<'_>) -> anyhow::Result<Vec<u8>> {
    ...
    let columns: Vec<ArrayRef> = vec![
        ...
        col(event.kind),
        col(event.source),
        opt(event.asset_id.map(|id| id.to_string())),
