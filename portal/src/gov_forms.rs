...
//!
//! `GET /app/forms` lists every vendored form from the registry;
//! `GET /app/forms/<form_code>.pdf` serves the canonical blank.
//! Both sit inside the `/app` auth + policy stack — the embedded Rego policy rule
...
/// `GET /app/forms/:file` — download one blank form. `:file` is
...
#[cfg(test)]
mod tests {
    use super::{download_get, GovFormsState};
    use axum::extract::{Path, State};
