...
//!
//! Every row supplied by the archive source serializes to a JSON object, so we
//! can build an Arrow `RecordBatch` for any table by:
//!
//! 1. Serializing every row to a JSON object.
//! 2. Taking the first row's keys as the column set.
...
//!
//! The Surreal source first calls `Value::into_json_value`: record links become
//! their stable `SurrealQL` `table:key` string and datetimes become RFC 3339
//! strings. Both are therefore stable `STRING` values in the analytical lake,
//! rather than SDK-shaped JSON objects.
...
mod tests {
    ...
    fn array_columns_are_json_serialized_strings() {
        let rows = vec![Row {
            ...
            name: "Cancer".into(),
            active: true,
            roles: vec!["lawyer".into(), "admin".into()],
        ...
        let column = batch
            ...
            .unwrap();
        assert_eq!(column.value(0), r#"["lawyer","admin"]"#);
    }
}
