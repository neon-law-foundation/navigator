...
//!
//! A target is a Rust triple plus the reader-facing platform label. The
//! object key a binary lands at in the public assets bucket
...
//! `asset_url(&lsp_binary_key(triple))`, which resolves to the public
...
/// The platforms a Zed (or any LSP-aware editor) user runs on: macOS on
/// both architectures and Linux on both architectures. A `navigator lsp
/// publish` pushes whichever of these it finds built; the page renders a
...
/// lives at: `lsp/<triple>/navigator-lsp`. Stable "latest" path — a
/// re-publish overwrites it, so the publisher stamps a *bounded*
/// `Cache-Control`, never `immutable`.
#[must_use]
pub fn lsp_binary_key(triple: &str) -> String {
    format!("lsp/{triple}/navigator-lsp")
}
...
mod tests {
    ...
    fn key_is_triple_scoped_under_lsp() {
        assert_eq!(
            lsp_binary_key("aarch64-apple-darwin"),
            "lsp/aarch64-apple-darwin/navigator-lsp"
        );
    }

    #[test]
    fn registry_covers_mac_and_linux_on_both_arches() {
