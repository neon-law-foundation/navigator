...
async fn seed_notation(world: &mut TrustWorld, name: String, email: String) {
    ...
    let person =
        store::test_support::ensure_person(&surreal, &store::persons::NewPerson::new(name, email))
            .await;
    let proj = store::test_support::seed_project(&surreal, "trust matter").await;
...
async fn attorney_approves_and_sends(world: &mut TrustWorld) {
    // The walked trust parks at the `lawyer_review` human gate — no render
    // on the settlor's final answer. Lawyer approves (renders + persists the
    // trust instrument at `generate_pdf__trust_pdf`) then sends it through
    // the generalized e-signature path.
    let nid = world.notation_id();
    ...
    // the assembled trust instrument is rendered. Follow it so the assertions
    // below read the document rather than an empty redirect body.
    let resp = world
        .app()
        .oneshot(
            Request::builder()
...
async fn assert_signature_request_id(world: &mut TrustWorld) {
    let surreal = features::shared_surreal().await;
    let request_id = store::signatures::request_id_for_notation(&surreal, world.notation_id())
        .await
        .unwrap();
    assert!(
        request_id.is_some(),
