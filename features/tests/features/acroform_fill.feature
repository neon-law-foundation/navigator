Feature: Fill a fillable government form, attorney-review-ready
  ...
  `generate_pdf__<form>` step (the same seam the retainer PDF uses) and
  the output is NEVER auto-filed: the workflow spec parks it at
  `lawyer_review` before any filing step, enforced by the review gate.

  Scenario: A generate_pdf__<form> step fills the form and the values survive the flatten
  ...
  Scenario: The form workflow cannot reach a filing step without lawyer_review
