Feature: MCP conversational notation creation
  ...
  Scenario: Full retainer walk over MCP advances the questionnaire to END
    When the LLM calls aida_create_notation for "onboarding__retainer" on that matter
    ...
    Then the MCP response status is "needs_answer"
    And the MCP next question is "custom_single_choice__governing_law"

    When the LLM calls aida_answer_notation with code "custom_single_choice__governing_law" value "nevada"
