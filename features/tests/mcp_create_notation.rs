...
struct McpWorld {
    ...
    /// one: `aida_create_notation` names an existing Project.
...
async fn seed_person(_world: &mut McpWorld, name: String, email: String) {
...
async fn open_matter(world: &mut McpWorld, email: String) {
    let surreal = features::shared_surreal().await;
    ...
    let entity_id = store::test_support::seed_entity(&features::shared_surreal().await).await;
...
#[when(regex = r#"^the LLM calls aida_create_notation for "([^"]+)" on that matter$"#)]
async fn create_notation(world: &mut McpWorld, template_code: String) {
