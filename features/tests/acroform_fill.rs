...
use workflows::{
    lawyer_review_gates_filing, DispatchingRuntime, DocumentPayload, InMemoryRuntime, MachineKind,
...
/// The form's workflow spec: fill → lawyer_review → file. The review
...
async fn worker_fills(world: &mut AcroWorld, entity_name: String, agent: String) {
    ...
    // Lands on generate_pdf__nv_articles → the dispatcher fills + persists.
    let next = rt
        .signal(MachineKind::Workflow, id, "start", Some(&payload))
        .await
        .unwrap();
    assert_eq!(next.as_str(), "generate_pdf__nv_articles");
}
...
#[then("the lawyer_review gate holds between fill and filing")]
async fn gate_holds(world: &mut AcroWorld) {
    let spec = world.spec.as_ref().expect("spec loaded");
    assert!(
        lawyer_review_gates_filing(spec).is_ok(),
        "the nv_articles spec must route every fill→file path through lawyer_review"
    );
}
