...
impl From<String> for CloseError {
...
pub async fn close_matter(
    ...
    store::projects::close_for_notation(surreal, notation_id).await?;
    Ok(())
}
...
mod tests {
    ...
    fn other_states_do_not_close_the_matter() {
        ...
        // lawyer review, a render, and END are not the firm's signature.
        assert!(!closes_matter(&StateName::from("client_signature")));
        assert!(!closes_matter(&StateName::from("lawyer_review")));
        assert!(!closes_matter(&StateName::from(
