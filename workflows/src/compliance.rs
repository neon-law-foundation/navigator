...
//! reachable without first crossing `lawyer_review`
//! ([`crate::lawyer_review_precedes_submission`]). So a `filings` row
...
impl From<String> for ComplianceError {
...
pub async fn dispatch_compliance(
    ...
    .await
    .map_err(|error| ComplianceError::Filing(error.to_string()))?;
    Ok(())
}
