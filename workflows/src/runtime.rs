...
//! the post-intake workflow that drives lawyer review, signing,
...
pub struct WorkflowEvent {
    ...
    /// The Person a [`SignalContext`] attributed this transition to,
    /// when the caller supplied one. `None` for a context-less
    /// `signal`. The durable journal (`notation_events`)
    /// resolves the concrete `acting_person_id` at append time; this
    /// field lets the in-memory runtime carry the same attribution so
    /// the dev/test path can assert it survived the call.
    #[serde(default)]
...
pub trait StateMachineRuntime: Send + Sync {
    ...
    /// Send a signal that carries a [`SignalContext`] attributing the
    /// transition to a specific Person. Unlike [`Self::signal`], every
    /// implementor must handle the context explicitly — there is no
    /// blanket default that silently drops it, because a durable runtime
    /// that discarded the actor would attribute a lawyer-driven workflow
    /// event to the wrong Person. The Restate adapter threads it onto the
    /// wire; the in-memory runtime records it on the event.
...
impl InMemoryRuntime {
    ...
    /// Shared body for [`Self::signal`] and [`Self::signal_with_context`]:
    /// advance the machine and record the transition, stamping
    /// `acting_person_id` onto the event when a context supplied one.
    fn advance(
        &self,
        kind: MachineKind,
        notation_id: Uuid,
...
impl StateMachineRuntime for InMemoryRuntime {
    ...
    async fn signal(
        ...
        notation_id: Uuid,
        condition: &str,
        // Ignored: `WorkflowEvent` has no payload column. The durable
        // projection lives in `RestateRuntime`; this runtime exists
        // for transition-order tests.
        _payload: Option<&str>,
    ) -> Result<StateName, WorkflowRuntimeError> {
        self.advance(kind, notation_id, condition, None)
    }
    ...
    async fn signal_with_context(
        ...
        self.advance(kind, notation_id, condition, Some(context.acting_person_id))
    }
...
";
    ...
    async fn signal_records_every_transition_in_the_event_log() {
        ...
        assert_eq!(events[0].to.as_str(), "lawyer_review__for_grantor");
        assert_eq!(events[0].condition, "created");
        assert_eq!(events[1].to.as_str(), "notarization__for_grantor");
    }

    #[tokio::test]
    async fn signal_with_context_records_the_acting_person_on_the_event() {
        // Regression for #252's runtime/context path: `signal_with_context`
        // must carry the actor through to the recorded event — the trait no
        // longer has a default that silently drops it. A plain `signal`
        // records no actor, proving the two paths stay distinct.
        use crate::runtime::SignalContext;
        let rt = InMemoryRuntime::new();
        let actor = Uuid::from_u128(555);
        StateMachineRuntime::start(&rt, MachineKind::Workflow, N1, &spec())
            .await
            .unwrap();
        StateMachineRuntime::signal_with_context(
            &rt,
        ...
        assert_eq!(
            ...
            Some(actor),
            "signal_with_context must preserve the actor, not silently drop it"
        );
        assert_eq!(
            events[1].acting_person_id, None,
            "a context-less signal records no actor"
        );
    }
