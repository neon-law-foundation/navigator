...
//! `ctx.run` — and asserts the durable `attestations` row lands. Runs
//! against an embedded, memory-backed store because the side effect
//! writes a real row.
//!
...
async fn null_attestor_writes_a_pending_row_with_no_transaction() {
    ...
    let deps = StepDeps::new(Arc::new(workflows::CapturingEmail::new()), storage)
        .with_surreal(surreal.clone())
    ...
    let row = store::attestations::by_notation(&surreal, notation_id)
        .await
        .unwrap()
...
async fn onchain_step_without_an_attestor_errors_clearly() {
    ...
    assert!(
        store::attestations::by_notation(&surreal, notation_id)
            .await
            .unwrap()
            .is_none(),
