...
//! `store` dependency so the LSP stays lean) must be exactly the
...
#[test]
fn rules_aggregate_types_match_the_store_registry() {
    let mut from_store = store::question_registry::QuestionType::aggregate_tokens();
    from_store.sort_unstable();
    let mut from_rules = rules::AGGREGATE_QUESTION_TYPES.to_vec();
    from_rules.sort_unstable();
    assert_eq!(
        from_rules, from_store,
        "rules::AGGREGATE_QUESTION_TYPES drifted from store::question_registry aggregates"
    );
}
