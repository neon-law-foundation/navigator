...
//!
//! `server/content/workshops/navigator/README.md` now teaches a concrete
...
//! exists, a workflow that stops being lawyer-gated. These tests pin every
...
fn llc_section() -> String {
    let readme = repo_file("server/content/workshops/navigator/README.md");
    let after = readme
...
fn every_command_the_section_documents_is_a_real_navigator_subcommand() {
    ...
    for (phrase, args) in [
        (
            "site notation create",
            ["site", "notation", "create"].as_slice(),
        ),
        (
            "site intake answer",
            ["site", "intake", "answer"].as_slice(),
        ),
        (
            "site notation status",
            ["site", "notation", "status"].as_slice(),
        ),
        (
            "site notation approve",
            ["site", "notation", "approve"].as_slice(),
        ),
        (
            "site notation document",
            ["site", "notation", "document"].as_slice(),
        ),
        ("site login", ["site", "login"].as_slice()),
    ] {
        assert!(
...
#[test]
fn the_lawyer_gated_filing_promise_holds_in_the_workflow() {
    // The section promises the matter ends at a lawyer-gated
    ...
    assert!(
        ...
        "`nv__llc_formation.yaml` no longer reaches `filing__nv_sos` — the workshop's \
         lawyer-gated-filing promise has drifted from the workflow",
    );
}
