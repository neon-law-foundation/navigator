...
//! flags that case so each create step matches the
...
/// 3. When a Config Sync repository is configured, enable Fleet config
///    management.
/// 4. Reconcile Fleet membership through the GKE cluster API.
/// 5. When configured, apply the Config Sync `RootSync`.
#[cfg(test)]
pub async fn ensure_autopilot_cluster(
    ...
    let cluster_outcome = ensure_autopilot_cluster_foundation(client, project_id, config).await?;
    ensure_cluster_integrations(client, project_id, config).await?;
    Ok(cluster_outcome)
}

/// Create the resources that must exist before a Kubernetes service account
/// can be granted access through the project's GKE workload identity pool.
pub async fn ensure_autopilot_cluster_foundation(
    client: &GcpClient,
    project_id: &str,
    config: &SetupConfig,
) -> SetupResult<EnsureOutcome> {
    ensure_static_ip(client, project_id, config).await?;
    ensure_cluster(client, project_id, config).await
}

/// Reconcile the already-created cluster's Fleet membership and apply optional
/// Config Sync only after the runtime workload identity bindings are in place.
pub async fn ensure_cluster_integrations(
    client: &GcpClient,
    project_id: &str,
    config: &SetupConfig,
) -> SetupResult<()> {
    if config.config_sync_repo.is_some() {
        ensure_config_management(client, project_id).await?;
    }
...
async fn ensure_fleet_membership(
    ...
    let result = client
        ...
        .await?;
    classify_fleet_membership(result)
}

async fn ensure_config_management(client: &GcpClient, project_id: &str) -> SetupResult<()> {
    let _ = client
        .shell_out(
            "gcloud",
            &[
...
fn classify_fleet_membership(result: ShellResult) -> SetupResult<EnsureOutcome> {
    const ALREADY_RECONCILED_MESSAGES: [&str; 2] = [
        "Modifying pre_registered fleet config is not supported",
        "Changing existing fleet membership is not supported",
    ];
    if result.exit != 0
        && ALREADY_RECONCILED_MESSAGES
            .iter()
            .any(|message| result.stderr.contains(message))
    {
        // The cluster update API cannot rewrite an output-only
        // `preRegistered` configuration or an existing membership. In both
        // cases the desired Fleet membership already exists.
        return Ok(EnsureOutcome::AlreadyExists);
    }
    classify(result, "enable cluster fleet membership")
}
...
mod tests {
    ...
    use super::super::client::{GcpClient, ShellResult, StaticToken};
    use super::super::SetupConfig;
    use super::{classify_fleet_membership, ensure_autopilot_cluster, EnsureOutcome};
    ...
    async fn dry_run_records_each_gcloud_invocation_in_order() {
        ...
        assert!(
            calls[1].url.contains("--enable-private-nodes"),
            "step 2 keeps Autopilot nodes off public IP quota: {}",
            calls[1].url
        );
        assert!(
            calls[1].url.contains("--enable-fleet"),
            "step 2 creates the Autopilot cluster as a Fleet member: {}",
            calls[1].url
        );
        assert!(
            calls[1]
                .url
                .contains(&format!("--network {}", config.vpc_name)),
            "step 2 attaches the deployment VPC: {}",
            calls[1].url
        );
        assert!(
            calls[1]
                .url
                .contains(&format!("--subnetwork {}", config.subnetwork_name)),
            "step 2 attaches the regional subnet: {}",
            calls[1].url
        );
        assert!(
        ...
        assert!(
            calls[3]
                .url
                .contains("container clusters update navigator-prod"),
            "step 4 reconciles Fleet through the GKE cluster API: {}",
            calls[3].url
        );
        assert!(
            calls[3].url.contains("--enable-fleet"),
            "step 4 enables Fleet on an existing cluster: {}",
            calls[3].url
        );
        assert!(
    ...
    async fn dry_run_skips_rootsync_when_no_repo_configured() {
        ...
        assert_eq!(
            calls.len(),
            3,
            "expected 3 shell-outs (Fleet membership but no Config Sync), got {calls:?}"
        );
        assert!(
            calls.iter().all(|c| {
                !c.url.starts_with("kubectl") && !c.url.contains("fleet config-management enable")
            }),
            "no Config Sync control-plane or RootSync call should be recorded: {calls:?}"
        );
    }
    ...
    #[test]
    fn pre_registered_fleet_membership_is_idempotent_success() {
        let result = ShellResult {
            exit: 1,
            command_line: "gcloud container clusters update example-a --enable-fleet".into(),
            stderr: "ERROR: (gcloud.container.clusters.update) INVALID_ARGUMENT: Modifying \
                     pre_registered fleet config is not supported."
                .into(),
        };

        assert_eq!(
            classify_fleet_membership(result).unwrap(),
            EnsureOutcome::AlreadyExists
        );
    }

    #[test]
    fn existing_fleet_membership_is_idempotent_success() {
        let result = ShellResult {
            exit: 1,
            command_line: "gcloud container clusters update neon-law-stg --enable-fleet".into(),
            stderr: "ERROR: (gcloud.container.clusters.update) INVALID_ARGUMENT: Changing \
                     existing fleet membership is not supported."
                .into(),
        };
    ...
    #[test]
    fn unrelated_fleet_invalid_argument_still_fails() {
        let result = ShellResult {
            exit: 1,
            command_line: "gcloud container clusters update broken --enable-fleet".into(),
            stderr: "ERROR: INVALID_ARGUMENT: fleet host project is invalid".into(),
        };

        let error = classify_fleet_membership(result).unwrap_err();
        assert!(
            error.to_string().contains("fleet host project is invalid"),
            "{error}"
        );
    }
}
