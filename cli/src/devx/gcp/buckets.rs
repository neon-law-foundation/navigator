...
/// Idempotently create a bucket that serves a static website directly.
///
/// Two things separate this from [`ensure_bucket`]. The `website` config makes
/// GCS resolve a directory request to `index.html`, which is what lets
/// `/attorneys/` answer `200` from `attorneys/index.html` instead of falling
/// through to the error page. And the bucket is intended to be world-readable
/// — [`ensure_public_read`] is the second half, kept separate so the anonymous
/// grant is its own reviewable call rather than a field on a create.
pub async fn ensure_website_bucket(
    client: &GcpClient,
    project_id: &str,
    name: &str,
    location: &str,
    ...
    match resp.status_u16() {
        ...
        409 => Ok(EnsureOutcome::AlreadyExists),
        other => Err(SetupError::BadStatus {
            operation: format!("create website bucket {name}"),
            status: other,
            body: resp.into_text(),
        }),
    }
...
/// Grant `allUsers` object-read on a published marketing bucket.
///
/// This is the one place in the workspace that deliberately makes a bucket
/// anonymously readable, and it is only ever called for the static marketing
/// sites, whose whole content is already published to the open web. It is a
/// read-only grant: `objectViewer` conveys no write, no delete, and no
/// listing of bucket metadata.
///
/// The write is get-merge-set against the live policy so an unrelated binding
/// added by hand is preserved, and a re-run that finds the member already
/// present makes no call at all.
pub async fn ensure_public_read(client: &GcpClient, name: &str) -> SetupResult<()> {
    const ROLE: &str = "roles/storage.objectViewer";
    const MEMBER: &str = "allUsers";

    let path = format!("/storage/v1/b/{name}/iam");
    let response = client.get(GcpService::Storage, &path).await?;
    let status = response.status_u16();
    if !(200..=299).contains(&status) {
        return Err(SetupError::BadStatus {
            operation: format!("read IAM policy for bucket {name}"),
            status,
    ...
    let bindings = policy
        ...
        .cloned()
        .unwrap_or_default();
    let already_granted = bindings.iter().any(|binding| {
    ...
    match resp.status_u16() {
        200..=299 => Ok(()),
        other => Err(SetupError::BadStatus {
            operation: format!("grant public read on bucket {name}"),
            status: other,
            body: resp.into_text(),
        }),
    }
