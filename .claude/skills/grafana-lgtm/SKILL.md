...
name: grafana-lgtm
description: >
  `navigator dev up` stands up the KIND stack with Grafana LGTM (Loki logs + Tempo traces + Prometheus metrics
...
- **`navigator dev up` does all the wiring — don't redo it by hand.** It applies the `lgtm` pod, port-forwards
