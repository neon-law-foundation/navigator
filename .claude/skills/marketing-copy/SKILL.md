--- name: marketing-copy description: > Authoring rules for client-facing marketing copy on neonlaw.com — firm pages
under `server/content/marketing/`, the views under `views/src/pages/`, and any public surface where the firm advertises
services (LinkedIn posts, YouTube content, talk titles when speaking *for the firm*). Trigger when adding or editing any
of those files, drafting a hero line, naming a new service tier, writing a service description, or quoting a fee. Three
load-bearing rules: (1) market only the firm's own value, never compare to or characterize "other lawyers / traditional
firms"; (2) no hyperbole or unsubstantiable superlatives; (3) attorney-advertising compliant across the firm's bar
admissions (CA, NV, WA) — no guarantees of results, no certification claims for SOC 2 / HIPAA, SLAs are service
commitments not outcome promises. Skip for non-marketing surfaces (internal docs, code comments, ADRs) and for
already-binding documents (signed retainers, filed pleadings) — those are reviewed by the attorney of record, not by
this skill. ---
...
**Bar admissions are facts, disclosed honestly.** Don't claim admissions the firm does not hold; don't hide the
admissions the firm does hold. The site names the jurisdictions each attorney is licensed in as a credential chip on
their `/team` profile (`views/src/team.rs`) and publishes no bar numbers. The footer's per-attorney licence list is a
seam a white-label deploy can fill through `brand.firm_attorneys`; this firm leaves it empty (`views/src/brand.rs`,
`FIRM_ATTORNEYS`).
...
1. **The seed data and templates** — `store/seeds/Address.yaml` (firm and Foundation addresses, suites, jurisdictions),
   `store/seeds/Credential.yaml` (each attorney's bar licences and numbers, which the site does not publish),
   `notation_templates/nonprofit/` (the Foundation is a **Nevada** 501(c)(3)), and the licensed-in credential chips in
   `views/src/team.rs`.
...
- **Transparent flat fees, no ranges.** Every published price is a fixed number (`$1,111` Nest, `$2,222/month` Nexus),
  never a range (`$3,500–$5,000`). Ranges read as evasive; fixed amounts are the brand.
- **Every firm fee uses the numerology ladder.** Our own legal fees are deliberate repdigit and repeating figures —
  `$44`, `$66`, `$555`, `$777`, `$1,111`, `$1,337`, `$2,222`, `$3,333`, `$8,888`, `$9,999` — not conventional round
  numbers. The live ladder is [`store/seeds/Product.yaml`](../../../store/seeds/Product.yaml); read it before quoting or
  inventing a price, and pick the figure from that system rather than rounding to a "clean" number. Third-party fees
  that pass through (state filing fees, court filing fees, the Ridgeview mailbox at `$300/year`, third-party
  registered-agent vendor fees) keep their actual amount and are labelled as pass-through — the numerology discipline is
  the firm's, and we do not bend a third party's fee to fit it. **Why:** legibility and memorability. A deliberate
  ladder reads as a considered scale; arbitrary numbers read as bespoke quoting in disguise.
...
- **Government fees are labelled and pass-through.** Every fee table on `nest.md`, `northstar.md`, and `nexus.md` has a
  "Government fees" column making clear what's included (legal fee only) vs. what's pass-through at cost (state filing
  fees, court filing fees, recorder fees). Never mark up government fees.
...
- **Litigation is the firm's featured practice, and it leads.** The firm litigates high-stakes technology disputes on a
  flat fee per phase, quoted after a case assessment. The firm publishes no per-service marketing pages — the home page
  states the practice (litigation and flat-fee transactional work) and prices through `/contact`. Litigation copy
  carries two things nothing else does: the attorney-advertising disclaimer on any results reference (*"Prior results do
  not guarantee a similar outcome; every matter turns on its own facts"*), and the referral-out path for a matter
  outside our lane, conflicted, or beyond capacity — written affirmatively, as part of the job, not as a way of turning
  someone away.

## Language — marketing is the one localized page

English is the only language ([`AGENTS.md`](../../../AGENTS.md)). Navigator publishes no translated page, and marketing
copy is written directly in the Rust module that renders it — there is no copy catalog, key lookup, or locale variant to
keep in sync. Write the English once, in the component.

## Where the rules live

- **Trigger files.** This skill should fire when authoring or editing any of:
  - `server/content/marketing/*.md` — the marketing markdown
  ...
  - `server/content/marketing/mission.md` — the canonical mission rendered at `/foundation/mission`
...
4. **Check every firm fee.** Grep the diff for every `$` amount. The firm publishes a fee on exactly one page — `/forms`
   on the firm host, whose flat attorney fee is prose in the `neon` crate's `firm_copy::forms` — plus a past-result
   recovery in a `/team` biography. Every other firm page quotes through `/contact` and publishes no fee. Third-party
   pass-throughs (Ridgeview `$300`, SoS filing fees, court fees, Copyright Office fees) keep their actual amount and are
   labelled as such.
...
7. **Confirm every concrete fact.** Re-read the diff for any address, suite number, state of incorporation, entity type,
   bar number, email, or fee. Each must trace to `store/seeds/Address.yaml`, a template, the bar strip, or an explicit
   answer from the user — never carried over from a sibling page on faith.
...
> Nevada, and Washington reviews your contracts and prepares your environment for SOC 2 and HIPAA audits. Every fee is
> quoted as a flat amount through the [contact page](/contact)."
