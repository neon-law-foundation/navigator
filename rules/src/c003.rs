...
//!
//! `portal::blog` derives the publish date from the `YYYYMMDD` filename
...
/// `portal::blog::parse_post_filename` does `NaiveDate::parse_from_str(.., "%Y%m%d")`
...
mod tests {
    ...
    fn rejects_an_impossible_calendar_date() {
        ...
        // and `portal::blog`'s strict parse would silently drop them — so the
