...
walker, no database, no async — easy to embed and easy to reuse outside Navigator (`compass` does exactly that with its
own rulebook).
...
`cli` and `compass` both depend on this crate. To add a new rule, follow the F-family pattern — one file under `src/`
with unit tests next to the impl — and then wire it into three places so it ships and the stability guard keeps catching
accidental reorders:
...
The engine doesn't care which crate the rule was authored in, so a Compass-specific rule lives in `compass/`, not here.
