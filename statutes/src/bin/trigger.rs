//! `statutes trigger` — the thin weekly `CronJob` entrypoint.
//!
//! Fires one `Statutes` workflow invocation against the Restate ingress,
//! then exits. The workflow key is the UTC run date, so a same-day re-fire
//! is idempotent: Restate runs a given workflow key at most once. The call
//! is one-way (`/send`): this process does no work beyond accepting the
//! invocation — Restate owns the retry schedule and runs the scrape → email
...
//! not. The shared [`workflows::start_workflow`] helper attaches the header
//! only when the token is present and non-empty, so the same binary works
...
async fn main() -> Result<()> {
    ...
    let _telemetry = telemetry::init("navigator-statutes-trigger");
    ...
    // workflow key, so a duplicate weekly fire is a no-op rather than a
    // double scrape.
    ...
    .await
    .context("triggering Statutes workflow")?;

    tracing::info!(%run_id, response = %body, "statutes workflow triggered");
    println!("triggered Statutes/{run_id}: {body}");
    Ok(())
}
