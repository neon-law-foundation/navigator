...
//! cannot violate it. See [the design](../../docs/project-repositories.md).
//!
...
/// their bytes live in `cloud::StorageService` (see `portal::git_lfs`)
...
/// Legacy UUID clone URL formatter retained for tests that exercise the
/// low-level repo store without a database row.
///
...
/// The public clone URL for a Project code: `<base>/projects/<code>.git`.
///
/// The code is stored on `projects.code` and is also the bare repo
/// directory name. The caller is responsible for passing a schema-validated
/// code; this function only formats the public URL.
#[must_use]
pub fn clone_url_for_code(base: &str, code: &str) -> String {
    format!("{}/projects/{code}.git", base.trim_end_matches('/'))
}
...
impl RepoStore {
    ...
    /// Filesystem path of a UUID-named bare repo used by low-level tests.
    /// Runtime project repos use [`Self::path_for_code`].
    #[must_use]
    pub fn path_for(&self, project_id: Uuid) -> PathBuf {
        self.root.join(format!("{project_id}.git"))
    }

    /// Filesystem path for a code-named bare repo: `<root>/<code>.git`.
    #[must_use]
    pub fn path_for_code(&self, code: &str) -> PathBuf {
        self.root.join(format!("{code}.git"))
    }
    ...
    /// Whether a code-named bare repo has been created.
    #[must_use]
    pub fn exists_code(&self, code: &str) -> bool {
        self.path_for_code(code).join("HEAD").is_file()
    }
    ...
    pub fn ensure(&self, project_id: Uuid) -> Result<PathBuf, RepoError> {
        ...
        self.ensure_path(&path, &project_id.to_string())
    }

    /// Create the Project's code-named append-only bare repo if absent.
    ///
    /// # Errors
    /// [`RepoError::Git`] if a `git` invocation fails, or
    /// [`RepoError::Io`] on a filesystem error.
    pub fn ensure_code(&self, code: &str) -> Result<PathBuf, RepoError> {
        let path = self.path_for_code(code);
        self.ensure_path(&path, code)
    }

    fn ensure_path(&self, path: &Path, repo_name: &str) -> Result<PathBuf, RepoError> {
        ...
        tracing::info!(repo = repo_name, path = %path_str, "created append-only bare repo");
        Ok(path.to_path_buf())
    }
    ...
    pub fn head_oid(&self, project_id: Uuid) -> Result<Option<String>, RepoError> {
        ...
        Self::head_oid_at(&repo)
    }

    /// The commit `main` points at in a code-named repo.
    ///
    /// # Errors
    /// [`RepoError::Git`] / [`RepoError::Io`] on failure.
    pub fn head_oid_code(&self, code: &str) -> Result<Option<String>, RepoError> {
        let repo = self.ensure_code(code)?;
        Self::head_oid_at(&repo)
    }

    fn head_oid_at(repo: &Path) -> Result<Option<String>, RepoError> {
    ...
    pub fn read_head_tree(&self, project_id: Uuid) -> Result<Vec<(String, Vec<u8>)>, RepoError> {
        ...
        let repo = self.ensure(project_id)?;
        Self::read_head_tree_at(&repo)
    }

    /// Every file at `main`'s tip in a code-named repo.
    ///
    /// # Errors
    /// [`RepoError::Git`] / [`RepoError::Io`] on failure.
    pub fn read_head_tree_code(&self, code: &str) -> Result<Vec<(String, Vec<u8>)>, RepoError> {
        if self.head_oid_code(code)?.is_none() {
            return Ok(Vec::new());
        }
        let repo = self.ensure_code(code)?;
        Self::read_head_tree_at(&repo)
    ...
    fn read_head_tree_at(repo: &Path) -> Result<Vec<(String, Vec<u8>)>, RepoError> {
        let repo_str = repo.to_string_lossy().into_owned();
    ...
    pub fn commit_as(
        ...
        let repo = self.ensure(project_id)?;
        Self::commit_as_at(&repo, author, message, files)
    }

    /// Append a commit to a code-named repo's `main`.
    ///
    /// # Errors
    /// [`RepoError::Git`] / [`RepoError::Io`] on failure.
    pub fn commit_as_code(
        &self,
        code: &str,
        author: Author<'_>,
        message: &str,
    ...
    fn commit_as_at(
        ...
        let parent = Self::head_oid_at(repo)?;
    ...
    /// caller enforces that — see `portal::expunge`). It does not delete any
    ...
    pub fn expunge_path(&self, project_id: Uuid, path: &str) -> Result<ExpungeOutcome, RepoError> {
        ...
        let outcome = Self::expunge_path_at(&repo, path, head_before)?;
        tracing::warn!(%project_id, path, "governed expunge: rewrote history to remove a path");
        Ok(outcome)
    }

    /// Governed expunge for a code-named repo.
    ///
    /// # Errors
    /// [`RepoError::Git`] / [`RepoError::Io`] on failure.
    pub fn expunge_path_code(&self, code: &str, path: &str) -> Result<ExpungeOutcome, RepoError> {
        let repo = self.ensure_code(code)?;
        let head_before = self.head_oid_code(code)?;
        let outcome = Self::expunge_path_at(&repo, path, head_before)?;
        tracing::warn!(
            code,
            path,
            "governed expunge: rewrote history to remove a path"
        );
        Ok(outcome)
    }
    ...
    fn expunge_path_at(
    ...
    ) -> Result<ExpungeOutcome, RepoError> {
        let repo_str = repo.to_string_lossy().into_owned();
        ...
        let head_after = Self::head_oid_at(repo)?;
        Ok(ExpungeOutcome {
            head_before,
            head_after,
            path: path.to_string(),
